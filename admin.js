(function () {
  ...
  const els = {
    ...
    deptControlList: document.getElementById('department_control_list'),
    deptControlStatus: document.getElementById('dept_control_status'),
    enableAllDeptsBtn: document.getElementById('btn_enable_all_depts'),
    disableAllDeptsBtn: document.getElementById('btn_disable_all_depts')
  };
  ...
  function initSideMenu() {
    ...
    // Default to dashboard on load (User Request: ignore previous session state)
    const lastActivePage = 'dashboard';
  ...
  function showPage(pageId) {
    ...
    // Refresh department control panel if that page is selected
    if (pageId === 'department-control') {
  ...
  async function loadDepartments() {
    try {
      ...
      // Load department availability after departments are loaded
  ...
  // ========== DEPARTMENT AVAILABILITY CONTROL ==========
  async function loadDepartmentAvailability() {
    try {
      ...
      if (data) {
        ...
        await saveDepartmentAvailability();
      }
      renderDepartmentControl();
    } catch (e) {
  ...
  async function saveDepartmentAvailability() {
    try {
      await db.ref('departmentAvailability').set(departmentAvailability);
      if (els.deptControlStatus) {
        els.deptControlStatus.textContent = '✓ Saved successfully';
        els.deptControlStatus.style.color = 'var(--success)';
        setTimeout(() => {
          els.deptControlStatus.textContent = '';
        }, 2000);
      }
    } catch (e) {
      console.error('Failed to save department availability:', e);
      if (els.deptControlStatus) {
        els.deptControlStatus.textContent = '✗ Failed to save';
        els.deptControlStatus.style.color = '#ff4444';
        setTimeout(() => {
          els.deptControlStatus.textContent = '';
        }, 3000);
      }
  ...
  function renderDepartmentControl() {
    if (!els.deptControlList) return;
    els.deptControlList.innerHTML = '';

    if (departments.length === 0) {
      els.deptControlList.innerHTML = '<div style="text-align:center;padding:20px;color:var(--muted);">No departments found. Add departments first.</div>';
      return;
    }

    departments.forEach(dept => {
      const isEnabled = departmentAvailability[dept.name] !== false; // Default to true
      const item = document.createElement('div');
      item.className = `dept-control-item ${!isEnabled ? 'disabled' : ''}`;
      item.innerHTML = `
        <div class="dept-control-info">
          <div class="dept-control-name">${dept.name}</div>
          <div class="dept-control-status ${isEnabled ? 'active' : ''}">
            ${isEnabled ? '✓ Available in app' : '✗ Disabled'}
          </div>
        </div>
        <button class="dept-toggle-switch ${isEnabled ? 'active' : ''}" 
                data-dept="${dept.name}" 
                aria-label="Toggle ${dept.name}">
        </button>
      `;

      const toggle = item.querySelector('.dept-toggle-switch');
      toggle.addEventListener('click', () => {
        const newState = !departmentAvailability[dept.name];
        departmentAvailability[dept.name] = newState;
        renderDepartmentControl();
        saveDepartmentAvailability();
      });

      els.deptControlList.appendChild(item);
    });
  }
  ...
  function init() {
    ...
    // Department Control Panel handlers
    if (els.enableAllDeptsBtn) {
      els.enableAllDeptsBtn.addEventListener('click', () => {
        departments.forEach(dept => {
          departmentAvailability[dept.name] = true;
        });
        renderDepartmentControl();
    ...
    if (els.disableAllDeptsBtn) {
      els.disableAllDeptsBtn.addEventListener('click', () => {
        departments.forEach(dept => {
          departmentAvailability[dept.name] = false;
        });
        renderDepartmentControl();
        saveDepartmentAvailability();
      });
